...
double filter_firFilter() { 
    double tempZ = 0;
    //iterating through every value of the input data and then calculating transfer function
    for(int32_t i = 0; i < xQueue.size; i++){
        tempZ = 0;
        //iterate through each coefficient, multiply and add to temp value to be added to new filtered arrayy
    ...
    for(int32_t i = 0; i < xQueue.size; i += DECIMATION_FACTOR) {
        //writing value to yQueue which is the filtered, decimated output of the FIR filter
...
double filter_iirFilter(uint16_t filterNumber) {
    ...
    for(int32_t i = 0; i < yQueue.size; i++){
        ...
        for(int32_t j = 0; j < IIR_COEF_COUNT; j++){
            if(j == 0) {
                tempb += pow(queue_readElementAt(&yQueue, i), -j) * iir_b_coeffs[filterNumber][j];          
            }
            else {
                tempa += pow(queue_readElementAt(&yQueue, i), -j) * iir_a_coeffs[filterNumber][j];
                tempb += pow(queue_readElementAt(&yQueue, i), -j) * iir_b_coeffs[filterNumber][j];          
            }            
        }
        tempZ = tempb/tempa;
...
double filter_computePower(uint16_t filterNumber, bool forceComputeFromScratch,
                           bool debugPrint){
    double sum = 0;
    //iterate through each value in the filter and compute its energy by squaring it and adding to the next value
    for(int32_t i = 0; i < OUTPUT_QUEUE_SIZE; i++){
        sum += queue_readElementAt(&outputQueue[filterNumber], i) * queue_readElementAt(&outputQueue[filterNumber], i);
    }   
...
const double *filter_getFirCoefficientArray() {return 0;}

// Returns the number of FIR coefficients.
uint32_t filter_getFirCoefficientCount() {return 0;}

// Returns the array of coefficients for a particular filter number.
const double *filter_getIirACoefficientArray(uint16_t filterNumber) {return 0;}

// Returns the number of A coefficients.
uint32_t filter_getIirACoefficientCount() {return 0;}

// Returns the array of coefficients for a particular filter number.
const double *filter_getIirBCoefficientArray(uint16_t filterNumber) {return 0;}

// Returns the number of B coefficients.
uint32_t filter_getIirBCoefficientCount() {return 0;}

// Returns the size of the yQueue.
uint32_t filter_getYQueueSize() {return 0;}

// Returns the decimation value.
uint16_t filter_getDecimationValue() {return 0;}

// Returns the address of xQueue.
queue_t *filter_getXQueue() {return 0; }

// Returns the address of yQueue.
queue_t *filter_getYQueue() { return 0; }

// Returns the address of zQueue for a specific filter number.
queue_t *filter_getZQueue(uint16_t filterNumber) { return 0; }

// Returns the address of the IIR output-queue for a specific filter-number.
queue_t *filter_getIirOutputQueue(uint16_t filterNumber) { return 0; }
// void filter_runTest();
